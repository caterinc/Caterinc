...
export async function GET(req: NextRequest) {
  ...
  const order = await prisma.order.findFirst({
    where: { OR: [{ orderNumber: numero }, { trackingCode: numero }] },
    select: {
      orderNumber: true,
      trackingCode: true,
      createdAt: true,
  ...
  if (!order) return NextResponse.json({ error: "Pedido não encontrado. Confira o código e tente novamente." }, { status: 404 });
